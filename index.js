...
function isTokenValid(config) {
  return config?.token && config?.expire && Date.now() < config.expire;
}
...
const DEFAULT_LOCATION = { latitude: 28.42995, longitude: 117.964691 };
...
async function fetchTokenByWxCode(wxCode) {
  ...
  const json = await request(url, { method: "POST", headers, body: "" });
...
async function printAsciiQRCode(uuid) {
  ...
  const buffer = Buffer.from(await (await fetch(qrUrl)).arrayBuffer());
  const resized = await sharp(buffer).resize({ width: 200 }).toBuffer();
  console.log("\n请使用微信扫描下方二维码：\n");
  console.log(await terminalImage.buffer(resized, { width: 200 }));
  console.log("\n（提示：此二维码为登录二维码）");
}
...
async function submitCheckIn(token, signature = "曹英翔", location = null) {
  const payload = {
    Id: 0,
    ThreadId: THREAD_ID,
    Signature: signature,
    RecordValues: [
      { FieldId: 1, Values: [], Texts: [], HasValue: false },
      {
        FieldId: 2,
        Values: [JSON.stringify(location || DEFAULT_LOCATION)],
  ...
  return request(url, { method: "POST", body: payload }, token);
}
...
async function main() {
  ...
  if (!isTokenValid(config)) {
    ...
    const { token, expire } = await fetchTokenByWxCode(wxCode);
    config = { token, expire };
    setConfig(config);
